...
const UpdateCard = ({ card, originalData }) => {
  ...
  const isUpdated = () => {
    const originalProperty = originalData.filter((item) => item.id === card.id);

    if (!originalProperty) return false;

    return (
      originalProperty.title !== card.title ||
      originalProperty.description !== card.description ||
      originalProperty.price !== card.price ||
      originalProperty.district !== card.district ||
      originalProperty.country !== card.country
    );
  };

  if (!isUpdated()) {
    return null; // Do not render if the property hasn't been updated
  }
    ...
    >
      ...
      <div className="relative w-full h-72 overflow-hidden group">
        ...
        <span className="absolute top-3 left-3 bg-blue-500 text-white text-xs font-bold px-2 py-1 rounded">
          Updated: {getTimeDifference(card.updatedAt)}
        </span>
