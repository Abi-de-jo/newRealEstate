...
const Property = mongoose.model("Residency", propertySchema);
const PropertyFilter = mongoose.model("Residency", propertySchema);
...
const fetchPropertiesByFilters = async (filters) => {
  try {
    ...
    console.log("Query:", JSON.stringify(query));
    const properties = await PropertyFilter.find(query);
    console.log("Found properties:", properties.length);
    return properties;
  } catch (error) {
    console.error("Error fetching properties by filters:", error.message);
        ...
        [
          {
            ...
            url: "https://t.me/rent_tbilisi_ge/9859" // Redirect to the specific Telegram post
          }
        ],
        // [{ text: "📝 Post an Ad",   url: "https://add-bot.vercel.app" }],
        [{ text: "📝 Post an Ad", web_app: { url: "https://add-bot.vercel.app" } }],
...
[{ text: "📝 Post an Ad", web_app: { url: "https://add-bot.vercel.app" } }],
  ...
  bot.action("all_properties", async (ctx) => {
    ...
    await sendFilteredProperties(ctx, properties); // Format and send the properties
  });
